...
import GQ from "country-flag-icons/react/3x2/GQ"
...
import PW from "country-flag-icons/react/3x2/PW"
import NR from "country-flag-icons/react/3x2/NR"
import FJ from "country-flag-icons/react/3x2/FJ"
import TV from "country-flag-icons/react/3x2/TV"
import TO from "country-flag-icons/react/3x2/TO"
...
import KI from "country-flag-icons/react/3x2/KI"
...
export function CountryFlag({ countryCode, className = "w-5 h-4" }: CountryFlagProps) {
  // 保护：如果 countryCode 未定义，显示 Globe 图标
  if (!countryCode || countryCode === "ROW") {
  ...
  // 未找到国旗
  if (!FlagComponent) {
    return <span className={`${className} inline-flex items-center justify-center bg-gray-200 text-xs font-medium rounded-sm`}>{countryCode}</span>
  }

  return <FlagComponent className={className} title="" />
}

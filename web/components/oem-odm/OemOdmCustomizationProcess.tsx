...
const rpx = (designValue: number) =>
  `calc(var(--rpx-customization) * ${designValue})`;
...
export function OemOdmCustomizationProcess({
  ...
  // 圆形按钮的拖拽处理 - 拖拽一次滚动一个 item
  const handleButtonDragEnd = (
    event: MouseEvent | TouchEvent | PointerEvent,
    info: PanInfo,
  ) => {
    const anglePerCard = 18;
    const threshold = 15; // 拖拽灵敏度阈值
    const currentValue = wheelRotation.get();
  ...
  // 点击按钮切换到下一个步骤 (保留作为兼容，或根据需求移除)
  const handleNextStep = () => {
    const anglePerCard = 18;
    const currentValue = wheelRotation.get();
    const targetValue = currentValue + anglePerCard;
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <path
                d="M271.04 192.126C271.04 192.126 60.72 122.821 0 0V479C0 479 87.12 321.965 271.04 280.733V321.965L374 235.991L271.04 145.63V192.126Z"
                fill="url(#paint0_linear_vector3)"
                fillOpacity="0.36"
              />
              <defs>
                <linearGradient
                  id="paint0_linear_vector3"
                  x1="359.5"
                  y1="240"
                  x2="-47"
                  y2="240"
          ...
          >
            ...
            >
              <path
                d="M154 109.5C154 109.5 34.5 70 0 0V273C0 273 49.5 183.5 154 160V183.5L212.5 134.5L154 83V109.5Z"
                fill="url(#paint0_linear_vector4)"
              />
              <defs>
                <linearGradient
                  id="paint0_linear_vector4"
                  x1="141.744"
                  y1="136.527"
                  x2="-255.256"
                  y2="136.527"
          ...
          <motion.div
            ...
            drag="y"
            dragConstraints={{ top: -3, bottom: 3 }}
            dragSnapToOrigin
            ...
            <svg width="100%" height="100%" viewBox="0 0 94 94" fill="none">
              ...
              <g filter="url(#filter0_d_button)">
                <circle
                  ...
                  cy="42.6"
                  r="34"
                  fill="url(#paint0_button_outer)"
                />
                <circle
                  cx="46.6"
                  cy="42.6"
                  r="18"
                  fill="url(#paint1_button_inner)"
                />
              </g>
            </svg>
          </motion.div>
              ...
              {titleSolidLines.map((line, index) => (
                <span key={index} className="flex items-baseline gap-2">
                  ...
                  {index === titleSolidLines.length - 1 && (
                    ...
                    >
                      <path
                        d="M36.5142 25.967L51.3344 40.6897C52.0601 41.4106 52.2166 42.4298 51.6839 42.9662L43.1354 51.5728C42.6027 52.1092 41.5826 51.9595 40.8569 51.2386L26.0368 36.5161L11.319 51.3344C10.5982 52.0601 9.5792 52.2166 9.04298 51.6839L0.438084 43.1355C-0.0981561 42.6027 0.0514422 41.5826 0.772211 40.8569L15.4899 26.0386L0.665549 11.3118C-0.0601414 10.5909 -0.216576 9.57167 0.316144 9.03533L8.86455 0.428656C9.39727 -0.107691 10.4174 0.0419398 11.1431 0.762852L25.9674 15.4896L40.6908 0.665554C41.4116 -0.0601398 42.4306 -0.216577 42.9668 0.316141L51.5717 8.86456C52.1079 9.39726 51.9583 10.4174 51.2376 11.1431L36.5142 25.967Z"
                        fill="#756F3F"
                      />
                    </svg>
                  )}
          ...
          <motion.div
            ...
            style={{
              left: rpx(-93),
              top: rpx(50 + titleSolidLines.length * 90 + 36),
              zIndex: 5,
            }}
            initial={{ opacity: 0, x: -30 }}
            whileInView={{ opacity: 1, x: 0 }}
          ...
          >
            <HollowText
              strokeColor="#423B06"
              strokeWidth={0.8}
              className="font-anaheim font-extrabold uppercase"
              style={{
                fontSize: rpx(90),
                lineHeight: rpx(46),
              }}
          ...
          >
            ...
            >
              <svg width="100%" height="100%" viewBox="0 0 77 77" fill="none">
                ...
                />
                <path
                  d="M33.876 59.1771L57.063 53.1223L51.0082 29.9353L46.1133 38.2901C21.0489 23.6053 25.0788 11.9381 25.0788 11.9381C25.0788 11.9381 13.7065 36.1375 38.7709 50.8223L33.876 59.1771Z"
                  fill="#BAB379"
                />
              </svg>
            </motion.div>
          ...
          >
            ...
            {cardPositions.map((card) => {
              ...
              let gradientStyle =
                "linear-gradient(180deg, #756F3F 0%, #CCC37C 100%)";

              if (isTopEdge) {
                // 第一个展示位（最上方卡片）：从左上到右下渐隐效果，参考图1
                // Stops: 17% -> 0% opacity, 29% -> 58% opacity, 53% -> 100% opacity
                gradientStyle =
                  "linear-gradient(174deg, rgba(117, 111, 63, 0) 5%, rgba(117, 111, 63, 0.58) 16%, #7D7745 38%, #948D55 60%, #CCC37C 100%)";
              } else if (isBottomEdge) {
                // 最后一个展示位（最下方卡片）：从下到上渐隐效果，参考图2
                // Stops: 71% -> 58% opacity, 83% -> 0% opacity
                gradientStyle =
                  "linear-gradient(174deg, #CCC37C 0%, #948D55 28%, #7D7745 47%, rgba(117, 111, 63, 0.58) 71%, rgba(117, 111, 63, 0) 94%)";
              }

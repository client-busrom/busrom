        ...
        {steps.slice(0, 5).map((step, index) => {
          ...
          return (
            <React.Fragment key={step.id}>
              ...
              <p
                className={`absolute font-josefin-sans text-black transition-all duration-300 pointer-events-none ${locale === "en" ? "whitespace-pre-line" : "whitespace-normal break-words"} ${isHovered ? "font-bold text-brand-dark-olive" : "font-medium"}`}
                style={{ left: vw(pos.x), top: vw(isHovered ? pos.hoverY : pos.y), width: vw(260), fontSize: vw(isHovered ? 36 : 32), lineHeight: isHovered ? vw(42) : vw(38) }}
              >
                {locale === "en" ? step.title : step.title.replace(/\n/g, " ")}
              </p>
            </React.Fragment>
          )
        })}
            ...
            return (
              <div key={step.id} className="relative flex items-center gap-6 z-10">
                ...
                <div className="translate-y-[1px]">
                  <p className={`font-josefin-sans font-semibold text-black leading-tight ${locale === "en" ? "whitespace-pre-line" : "whitespace-normal break-words"}`} style={{ fontSize: mvw(18) }}>
                    {locale === "en" ? step.title : step.title.replace(/\n/g, " ")}
                  </p>
                </div>
              </div>

...
export function FaqHeroSection({ data, locale }: FaqHeroSectionProps) {
  ...
  const handleNext = () => {
    ...
    // Step 1: Hide background
    setIsBgVisible(false);

    // Step 2: Delay and then move items
    setTimeout(() => {
      // Use monotonic increment to prevent collective entry animations
      setActiveIndex((prev) => prev + 1);

      // Step 3: Wait for movement to finish, then show background
      setTimeout(() => {
        setIsBgVisible(true);
      }, 500); // Start showing background as the card is landing for a snappier feel
    }, 250); // Delay for background fade-out
  };

  // Auto-slide logic with reset on change
  useEffect(() => {
    if (items.length <= 1) return;
  ...
  // Manual High-Performance Renderer for FAQ Lexical JSON
  ...
  const renderCta = () => {
    ...
    return (
      <Link
        ...
        style={{
          left: vw(1150), // Calibrated center-aligned: 960 + 190 offset
          top: vw(632),
          zIndex: 30,
        }}
      >
        ...
        <div
          className="absolute left-0 top-0 border-2 border-white bg-white/20 backdrop-blur-sm transition-all duration-500 ease-out pointer-events-none opacity-0 group-hover:opacity-100 flex items-center"
          style={{
            height: vw(circleSize),
  ...
  return (
    ...
    >
      {/* Background with Overlay and Sweep Animation */}
        ...
        {data.bgImage ? (
          <div className="relative w-full h-full">
            <OptimizedImage
              ...
              size="xlarge"
            />
            {/* Scale Glass Shimmer Effect */}
            <div className="absolute inset-0 z-10 pointer-events-none">
              {/* Subtle Texture Overlay (Scale pattern simulator) */}
              <div
              ...
              {[...Array(6)].map((_, i) => (
                <motion.div
                  ...
                  className="absolute w-[300px] h-[300px] bg-white/10 blur-[80px] rounded-full"
                />
              ))}

              {/* Occasional Sharp Refraction */}
              <motion.div
                animate={{
                  left: ["-100%", "200%"],
                  opacity: [0, 0.2, 0],
                ...
                className="absolute top-0 w-[100px] h-full bg-white/5 skew-x-[-35deg] blur-3xl"
              />
            </div>
          </div>
        ) : (
      ...
      <div className="relative z-10 w-full h-full overflow-hidden">
        {/* Carousel Items Positioning - Symmetrically Calibrated */}
          ...
          <AnimatePresence initial={false}>
            {displayItems.map(({ vIndex, pos, style }) => {
              ...
              return (
                ...
                >
                  {/* Background color fades in only for the center slot */}
                  <AnimatePresence>
                  ...
                  >
                    <div
                      className={`absolute inset-0 z-10 border-2 border-dashed ${isCenter ? "border-white" : "border-white/30"}`}
                      style={{ borderRadius: vw(300), borderWidth: vw(2) }}
                    />
                    <div
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center">
            <div
              className="rounded-full border-2 border-white flex items-start justify-center"
              style={{
                width: vw(19.5),
                height: vw(31.6),
                borderWidth: vw(2),

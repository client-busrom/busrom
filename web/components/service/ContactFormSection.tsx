...
export function ContactFormSection({
    ...
    try {
      ...
      setFormData((prev) => {
        if (field.validation?.multiple) {
          const existingUrls = Array.isArray(prev[fieldName])
            ? prev[fieldName]
            : [];
          return {
            ...prev,
    ...
    try {
      ...
      formConfig?.fields.forEach((field) => {
        const optionsWithCustom =
          field.options?.filter((o: any) => o.hasCustomInput) || [];

        if (optionsWithCustom.length > 0) {
          if (
            field.fieldType === "checkbox" &&
            Array.isArray(processedData[field.fieldName])
          ) {
            processedData[field.fieldName] = processedData[field.fieldName].map(
              (val: string) => {
                const hasCustom = optionsWithCustom.some(
                  (o: any) => o.value === val,
                );
                if (hasCustom) {
                  const customVal =
                    processedData[`${field.fieldName}_custom_${val}`];
                  if (customVal) {
            ...
            ["radio", "select", "checkbox"].includes(field.fieldType)
          ) {
  ...
  const getFieldsByType = () => {
    ...
    const sorted = [...formConfig.fields].sort(
      (a, b) => (a.order || 0) - (b.order || 0),
    );

    return {
      nameEmail: sorted.filter(
        (f) =>
          f.fieldType === "text" ||
          f.fieldType === "email" ||
          f.fieldType === "tel",
              ...
              {field.options?.map((option) => {
                ...
                return (
                  <label
                    key={option.value}
                    className={
                      isSelected ? checkboxItemActiveClass : checkboxItemClass
                    }
                  >
                    <div
    ...
    >
      ...
      <div className="lg:hidden relative">
        ...
        <div className="relative px-5 py-12">
          {/* Header */}
          <div className="mb-14">
            <h2 className="font-anaheim font-extrabold text-[42px] leading-[1.1] text-white mb-6 whitespace-pre-line">
              {title.includes('\n') ? (
                <>
                  {title.split('\n')[0].split(' ').slice(0, -1).join(' ')}{' '}
                  <span className="text-[#FFF071]">{title.split('\n')[0].split(' ').slice(-1)}</span>
                  <br />
                  {title.split('\n')[1]}
                </>
              ) : title}
            ...
            {displayName && (
              <p className="font-anaheim font-extrabold text-[22px] text-[#FFF071] mb-4">
                {displayName}
              </p>
            )}
            {description && (
              <p className="font-anaheim font-normal text-white/90 text-[16px] leading-relaxed">
                {description}
              </p>
            )}
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-white/20 backdrop-blur-sm rounded-[12px] p-6 text-center">
              ...
              <h3 className="text-lg font-anaheim font-bold text-white mb-2">
                Success!
              </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-[12px]">
              ...
              <button
                ...
                className="w-full min-h-[52px] py-3 rounded-[100px] bg-[#B2A224] text-white font-anaheim font-semibold text-[16px] leading-tight hover:bg-[#9A8C1E] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center text-center"
              >
                {submitting
                  ? "Sending..."
          ...
          <div className="mt-6 pt-5 border-t border-white/20">
            <div className="mb-3">
              <p className="font-anaheim font-semibold text-[12px] text-white/70">
                Email
              </p>
              <p className="font-anaheim font-semibold text-[16px] text-white">
                {email}
              </p>
            </div>
            <div className="mb-3">
              ...
              <p className="font-anaheim font-semibold text-[17px] text-white">
                {phone}
              </p>
            </div>
            ...
            <div className="relative w-full flex">
              ...
              >
                <h2
                  ...
                  style={{ fontSize: vw(85), lineHeight: vw(98) }}
                >
                  {title.includes('\n') ? (
                    <>
                      <span
                ...
                <div style={{ marginTop: vw(100) }}>
                  ...
                  <a
                    href={`mailto:${email}`}
                    className="font-anaheim font-semibold text-white underline"
                  ...
                  <p
                    className="font-anaheim font-semibold text-white/70"
                    style={{ fontSize: vw(24), marginTop: vw(40) }}
                  >
                    Phone / WhatsApp
                  </p>
                  <a

...
const HeroOrbit = React.memo(() => {
  const points = React.useMemo(() => {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      xPoints.push(vw(204 + x));
      yPoints.push(vw(84 + y));
    }
  ...
  if (isTabletOrMobile) {
    return (
      <section className="relative w-full bg-[#57522a] pt-[120px] pb-[140px] px-6 sm:px-12 overflow-hidden">
        ...
        <div className="relative z-10 w-full h-full flex flex-col md:flex-row items-start md:items-end gap-12 md:gap-16 lg:gap-24">
    ...
    >
      ...
      <div className="relative z-10 w-full h-full">
        ...
        <motion.div
          ...
          className={`absolute flex flex-col items-start font-josefin-sans ${locale === "en" ? "whitespace-pre-wrap" : "overflow-y-auto"}`}
          style={{
            left: vw(820),
            top: vw(120),
            zIndex: 10,
            maxHeight: locale === "en" ? "none" : "8em", // 4 lines * 1.5 line-height + breathing room
            width: vw(600), // Ensure width is bound so it can wrap
        ...
        >
          <div style={{ fontSize: vw(29), lineHeight: 1.5, fontWeight: 600, textShadow: `0 ${vw(4)} ${vw(11)} #565020` }}>
            {data.descriptionNodes ? data.descriptionNodes.map((node, idx) => {
              if (node.type === "linebreak") return locale === "en" ? <br key={idx} /> : " ";
        ...
        <motion.h2
          ...
          className="absolute font-bold font-josefin-sans tracking-tight drop-shadow-lg text-white"
          style={{
            left: vw(842),
            top: vw(518),
            fontSize: vw(60),
        ...
        <motion.div
          ...
          className={`absolute ${locale === "en" ? "" : "overflow-y-auto"}`}
          style={{
            left: vw(159),
            top: vw(644),
            width: vw(639),
            zIndex: 10,
            maxHeight: locale === "en" ? "none" : "8em", // 4 lines * 1.5 line-height
            scrollbarWidth: locale === "en" ? "none" : "thin",
            scrollbarColor: "rgba(255,255,255,0.3) transparent",
            paddingRight: locale === "en" ? 0 : "4px"
          }}
        >
          <p
            className={`text-white opacity-90 drop-shadow-md font-josefin-sans ${locale === "en" ? "whitespace-pre-wrap" : ""}`}
            style={{
              fontSize: vw(24),
              lineHeight: 1.5
            }}
...
function HeroFeatureItem({ text, delay, width, x, y, starX, starY }: { text: string, delay: number, width: string, x: number, y: number, starX: number, starY: number }) {
  return (
    <motion.div
      ...
      className="absolute flex justify-center rounded-[49px] border border-[#ffec51] bg-[#3a20008c] shadow-[0_0_12px_#ffbf51] px-8"
      style={{
        paddingTop: vw(18),    // Adjust this to fix font baseline offset
        paddingBottom: vw(12), // Adjust this to fix font baseline offset (total 30 to maintain 78 height)
        width: "max-content",
        minWidth: width,
        left: vw(x),
        top: vw(y)
    ...
    >
      <span
        className="text-[#ffec51] font-normal font-josefin-sans flex justify-center text-center whitespace-nowrap"
        style={{
          fontSize: vw(40),

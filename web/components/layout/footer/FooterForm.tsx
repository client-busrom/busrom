  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const successMsg = formConfig?.successMessage || (locale === 'zh' ? '提交成功！' : 'Submitted successfully!');
        onSuccess(successMsg);
      } else {
        const data = await response.json();
        setSubmitStatus('error');
        setErrorMessage(data.error || (locale === 'zh' ? '提交失败' : 'Submission failed'));
        setTurnstileKey(prev => prev + 1);
      }
    } catch (error) {
      setSubmitStatus('error');
    ...
    <div className="w-full lg:w-[45%]">
      ...
      <form className="flex flex-col gap-6 lg:gap-8" onSubmit={handleSubmit}>
        ...
        <div className="mt-4 flex flex-col gap-3">
          ...
          >
            {isSubmitting ? (formConfig?.submittingText || '...') : (formConfig?.submitButtonText || content.form.buttonText)}
          </Button>
        </div>
      </form>
    </div>

...
const MagnifierIcon = ({ size = 40 }: { size?: number }) => (
  <svg
    ...
    style={{ width: vw(size), height: vw(size) }}
...
const RightBoxFrame = () => (
  ...
  >
    <path
      d="M49 0.5H365C391.786 0.5 413.5 22.2373 413.5 49.0234V268.703C413.5 293.991 393 314.5 367.713 314.5C341.362 314.5 320 335.862 320 362.213V365.434C320 386.457 302.957 403.5 281.934 403.5H49C22.2142 403.5 0.5 381.786 0.5 355V49C0.5 22.2142 22.2142 0.5 49 0.5Z"
      stroke="white"
    />
    {/* Circular Background */}
    <path
      d="M398.829 336.171C414.346 351.688 414.346 376.845 398.829 392.362C383.312 407.879 358.154 407.879 342.637 392.362C327.121 376.846 327.121 351.688 342.637 336.171C358.154 320.654 383.312 320.654 398.829 336.171Z"
...
const ViewMoreButton = ({
  ...
  return (
    <button
      ...
      style={{ width: vw(230), height: vw(86) }}
    >
      <svg
        className="absolute inset-0 w-full h-full pointer-events-none"
        viewBox="0 0 230 86"
      ...
      >
        <rect
          ...
          height="71"
          rx="35.5"
          className="fill-white/70 group-hover:fill-[#756F3F] transition-colors duration-300"
        />
        <circle
          cx="42.7332"
          cy="42.7332"
      ...
      <span
        className="absolute font-anaheim font-semibold text-[#756F3F] tracking-wider transition-colors duration-300 group-hover:text-white"
        style={{ left: vw(85), top: vw(28), fontSize: vw(24) }}
      >
        {text}
      </span>
    </button>
...
export function ApplicationHeroSection({
  ...
  const slide = slides[activeSlide] || {
    gradientTitle: "Professional",
    mainTitle: "Hardware Solutions For Every Space",
    subtitle: "",
  };
        ...
        >
          ...
          >
            <div
              ...
              style={{ top: vw(30), right: vw(30), zIndex: 10 }}
            >
              <MagnifierIcon size={60} />
            </div>
            <button
              onClick={() => swapAndNext(3)}
              className="absolute inset-0 w-full h-full z-20 cursor-pointer rounded-[60px]"
            />
          </div>
          ...
          >
            ...
            <div
              className="absolute pointer-events-none"
              style={{ left: vw(26), top: vw(339) }}
            >
              <span
                className="font-anaheim font-medium text-white tracking-widest"
                style={{ fontSize: vw(28), lineHeight: vw(40) }}
              >
                {rightBoxText}
              </span>
            </div>
            <button
              onClick={() => swapAndNext(1)}
              className="absolute inset-0 w-full h-full z-20 cursor-pointer rounded-[39px]"
            />
          </div>
          ...
          <div
            ...
            style={{ left: vw(160), top: vw(132), zIndex: 20 }}
          >
            {/* Header / Title */}
            <div className="flex items-center" style={{ marginBottom: vw(10) }}>
              <span
                className="font-anaheim font-medium text-white tracking-widest"
                style={{ fontSize: vw(20), lineHeight: vw(40) }}
              >
                {title}
              </span>
              <div
              ...
              >
                ...
                >
                  ...
                  <text
                    ...
                    fill="transparent"
                    stroke={`url(#grad-desk-${activeSlide})`}
                    strokeWidth="2"
                    className="font-jomhuria"
                    style={{ fontSize: vw(200), paintOrder: "stroke fill" }}
                  >
                    {slide.gradientTitle.split("\n").map((line, i) => (
                      <tspan x="0" dy={i === 0 ? "0" : "0.8em"} key={i}>
                        {line}
                      </tspan>
                    ))}
                  </text>
                ...
                <div className="flex">
                  <div
                    style={{
                      width: vw(2),
                      height: vw(144),
                      backgroundColor: "#E6E0AA",
                      flexShrink: 0,
                      marginLeft: vw(41),
                    }}
                  />
                  <p
                    className="font-jomhuria text-white"
                    style={{
                      fontSize: vw(64),
                      lineHeight: vw(48),
                      width: vw(385),
          ...
          <div
            className="absolute"
            style={{ left: vw(960), top: vw(1036), zIndex: 20 }}
          >
      ...
      <div className="block lg:hidden w-full px-6 pt-24 pb-16 bg-[#211C0B] min-h-screen">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <text
              ...
              strokeWidth="2"
              className="font-jomhuria text-[80px]"
        ...
        <div className="flex gap-3 mb-6">
          ...
          <AnimatePresence mode="wait">
            <motion.p
              ...
              exit={{ opacity: 0 }}
              className="font-jomhuria text-white text-[30px] leading-[1.1] whitespace-pre-line"
            >
              {slide.subtitle}
            </motion.p>
          </AnimatePresence>
        ...
        <div className="grid grid-cols-3 gap-2 mb-6">
          {[1, 2, 3].map((slotIdx) => (
            <button
              key={slotIdx}
              onClick={() => swapAndNext(slotIdx)}
              className="aspect-square rounded-xl overflow-hidden relative group"
            >
              {img(slotIdx) ? (
                <OptimizedImage
                  image={img(slotIdx) as any}
                  alt={`App ${slotIdx + 1}`}
                  size="thumbnail"
                  className="w-full h-full object-cover"
                />
        ...
        <div className="flex flex-col items-center gap-4">
          <button
            onClick={nextSlide}
            className="w-[180px] h-[50px] rounded-full bg-white/70 flex items-center justify-center"
          >
            <span className="font-anaheim font-semibold text-[#756F3F] text-sm">
              {bottomBoxText}
            </span>
          </button>

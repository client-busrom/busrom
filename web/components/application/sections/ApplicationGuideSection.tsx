...
export function ApplicationGuideSection({
  ...
  const renderSegments = (segments: RichTextSegment[]) => {
    return segments.map((s, i) => (
      <React.Fragment key={i}>
        {s.linebreak ? <br /> : <span className={s.bold ? "font-bold" : ""}>{s.text}</span>}
      </React.Fragment>
    ))
  }
  ...
  const ArrowIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
    <svg viewBox="0 0 350 20" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} style={style}>
      <path d="M335 2L348 10L335 18" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
      <path d="M0 10H348" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
    </svg>
  )

  const CircleArrow = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
    <svg viewBox="0 0 46 21" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} style={style}>
      <path d="M36 2L44 10.5L36 19" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
      <path d="M1 10.5H44" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
    </svg>
  )
  ...
  const GuideButton = ({ text, url }: { text: string; url: string }) => (
    ...
    >
      ...
      <span className="font-bold relative z-10 text-black" style={{ fontSize: vw(32), marginRight: vw(27) }}>{text}</span>
      
      <motion.div 
        className="relative z-10 flex items-center justify-center rounded-full"
        style={{ width: vw(78), height: vw(78) }}
        variants={{
        ...
        transition={{ duration: 0.3 }}
      >
        <CircleArrow style={{ width: vw(46), height: vw(21) }} />
      </motion.div>
    </motion.a>
  )
  ...
  const titleConfig = {
    ...
    translateX: 139,        // Horizontal Offset from anchor (px)
    translateXInner: -80,   // Horizontal Offset for Inner White Text (px)
    lineHeight: 140,        // Absolute Line Height (px)
    width: 800              // Title container width (px)
  }

  const arrowConfig = {
    marginLeft: -380,       // Horizontal Offset from Center (50%) (px)
    marginTop: -260,        // Vertical Offset from Center (50%) (px)
    width: 163,             // Button Width (px)
    height: 89.5            // Button Height (px)
  }
  // --------------------------------------------------------------------------
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{ transform: `translateY(${vw(100)})` }} // Content Shifted Down 100px
      >
        
        {/* The Entire Group Wrapper centered on page 50% */}
        <div className="relative h-full flex items-center justify-center" style={{ width: vw(1300) }}>
          ...
          >
             <h2 
               ...
               style={{ fontSize: vw(110), lineHeight: vw(titleConfig.lineHeight) }}
             >
          ...
          >
            <CircleArrow style={{ width: vw(46), height: vw(21), color: '#000' }} />
          </div>
          ...
          >
            {image ? (
              <img src={image} alt="Guide" className="w-full h-full object-cover" />
            ) : (
            ...
            >
                ...
                >
                  <CircleArrow style={{ width: vw(46), height: vw(21), color: '#fff' }} />
                </div>
            </div>
          </div>
          ...
          <div 
              ...
              style={{ 
                ...
                left: '55%', 
                top: '50%',
                transform: `translate(${vw(100)}, -50%)`,
                backgroundColor: 'rgba(146, 137, 62, 0.58)',
                padding: vw(50),
                gap: vw(40)
              }}
          >
              <p className="text-white text-center leading-relaxed whitespace-pre-line" style={{ fontSize: vw(32) }}>
                  {renderSegments(description)}
              </p>
              
              <div className="flex flex-col gap-4" style={{ gap: vw(20) }}>

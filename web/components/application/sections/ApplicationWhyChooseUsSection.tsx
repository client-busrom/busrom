...
export function ApplicationWhyChooseUsSection({
  ...
  return (
    <>
      ...
      <section className="lg:hidden w-full bg-[#FFFDF8] py-8 px-4 select-none flex flex-col items-center overflow-hidden">
        {/* Header */}
        <div className="w-full max-w-xl text-center mb-6">
          <HollowText
            strokeColor="#756F3F"
            strokeWidth={1.5}
            className="font-extrabold text-5xl md:text-7xl block mb-1"
        ...
        <div className="w-full max-w-xl bg-[#A59E69] rounded-2xl p-4 md:p-6 flex flex-col gap-4 relative shadow-lg">
          {/* Slider State with AnimatePresence */}
          <AnimatePresence mode="wait">
            <motion.div
              key={currentItem.id}
              initial={{ opacity: 0, x: 20 }}
            ...
            >
              {/* Main Card Image (Left image from desktop) with leaf-like border radius - shorter aspect ratio on mobile */}
              <div
                className="w-full h-36 md:h-52 overflow-hidden shadow-md"
                style={{
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-grow h-[2px] bg-[#544D0F]" />
              </div>

              {/* Slide Title - no pre-line to auto wrap */}
              <div>
                <h5
                  className="text-lg md:text-xl font-bold text-[#FFF6B1] leading-snug break-words"
                  style={{
                    fontFamily: "var(--font-black-han-sans), sans-serif",
                  }}
                >
          ...
          <div className="flex justify-between items-center mt-2 pt-2 border-t border-white/20">
              ...
              {items.map((item, idx) => (
                <button
                  ...
                  className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                    currentIndex % items.length === idx ? "bg-[#FFF6B1] w-5" : "bg-white/40"
                  }`}
                />
              ))}
            </div>
            ...
            <div className="flex gap-2">
              <button
                onClick={handlePrev}
                className="w-8 h-8 rounded-full flex items-center justify-center bg-white/20 border border-white/60 text-white hover:bg-white hover:text-[#A59E69] transition-all"
              >

...
const BANNER_7_ASSETS = {
  ...
  imageBox: {
    ...
    diamonds: [
      { id: "top", src_idx: 1, frame: "/home/hero-banner/banner-7/hero-banner-7-2.svg", mask: "/home/hero-banner/banner-7/hero-banner-7-2-image.svg", x: 200, width: 566, height: 366, maskW: 528, maskH: 347, alignY: "top", innerAlignY: "top", zIndex: 20 },
      { id: "middle", src_idx: 2, frame: "/home/hero-banner/banner-7/hero-banner-7-3.svg", mask: "/home/hero-banner/banner-7/hero-banner-7-3-image.svg", x: 557.06, width: 378, height: 378, maskW: 360, maskH: 360, alignY: "center", innerAlignY: "center", zIndex: 21 },
      { id: "bottom", src_idx: 3, frame: "/home/hero-banner/banner-7/hero-banner-7-4.svg", mask: "/home/hero-banner/banner-7/hero-banner-7-4-image.svg", x: 200, width: 566, height: 377, maskW: 528, maskH: 358, alignY: "bottom", innerAlignY: "bottom", zIndex: 22 },
    ],
  },
  mobile: {
...
const HeroBanner7: FC<BannerProps> = ({ data }) => {
  ...
  return (
    <section 
      className="relative w-full h-full lg:aspect-[1920/922] overflow-hidden"
    ...
    >
      ...
      <div className="hidden lg:block absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none"
        ...
        {BANNER_7_ASSETS.lightBeams.map((beam, index) => (
          <div key={index} className="absolute pointer-events-none" style={{
            right: rpx(beam.right), top: rpx(beam.y), width: rpx(beam.width), height: rpx(beam.height),
            opacity: beam.opacity, transform: `rotate(${beam.rotation}deg)`, transformOrigin: "left top",
            background: "linear-gradient(to right, transparent, #FFED5B 20%, #FFED5B 80%, transparent)",
            zIndex: 5,
          }} />
        ))}

        <div className="absolute z-10" style={{
          left: rpx(BANNER_7_ASSETS.imageMain.x), top: "50%", transform: "translateY(-50%)",
          width: rpx(BANNER_7_ASSETS.imageMain.width), height: rpx(BANNER_7_ASSETS.imageMain.height),
          maskImage: `url(${BANNER_7_ASSETS.imageMain.mask})`, WebkitMaskImage: `url(${BANNER_7_ASSETS.imageMain.mask})`,
          maskSize: "100% 100%",
        }}>
          ...
          {BANNER_7_ASSETS.imageBox.diamonds.map((diamond) => (
            <div key={diamond.id} className="absolute" style={{
              ...
              zIndex: diamond.zIndex,
               ...
               <div className="absolute inset-0 z-0" style={{ backgroundImage: `url(${diamond.frame})`, backgroundSize: "100% 100%" }} />
               <div className="absolute z-10 left-1/2 -translate-x-1/2" style={{
                  width: rpx(diamond.maskW), height: rpx(diamond.maskH),
      ...
      <div className="lg:hidden absolute inset-0 z-30 overflow-y-auto">
        <div className="flex flex-col items-center justify-center min-h-full py-4 px-6 gap-3">
          ...
          <div className="relative z-30 flex flex-col items-center shrink-0">
            {titleParts.map((line, idx) => (
              <h1 key={idx} className="font-paytone-one leading-[1.0] text-center text-3xl md:text-5xl" 
                  style={{ color: idx === 0 ? "#FFFFFF" : "#433E12", WebkitTextStroke: "1px #000000", paintOrder: "stroke fill" }}>
                {line}
              </h1>
            ))}
          </div>
          ...
          <div className="relative z-40 flex flex-col items-center gap-1.5 w-full shrink-0">
            {featuresList.map((feature, idx) => (
              <div key={idx} className="bg-[#E9E2A0] px-6 py-1.5 md:px-8 md:py-2.5 rounded-full text-black shadow-sm border border-black/5">
                <span className="font-montserrat font-bold text-[10px] md:text-lg tracking-wider block text-center min-w-[100px] md:min-w-[150px]">
                  {feature}
                </span>
              </div>
            ))}
          ...
          <div className="relative z-30 text-center px-4 pb-12 shrink-0">
            <p className="font-paytone-one text-white text-lg md:text-2xl opacity-90 leading-tight">
              {subtitleParts.join(" ")}
            </p>
          </div>
        </div>

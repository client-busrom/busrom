            ...
            {items.map((item: any, index: number) => (
              <CarouselItem key={index} className="pl-6 basis-auto">
                <div className="space-y-4">
                  ...
                  {(item.media?.variants?.large || item.media?.variants?.desktop || item.media?.url) && (
                    <div className="relative w-64 h-64 md:w-[488px] md:h-[488px] rounded-2xl overflow-hidden">
                      <Image
                        src={item.media.variants?.large || item.media.variants?.desktop || item.media.variants?.tablet || item.media.variants?.card || item.media.variants?.thumbnail || item.media.url}
                        alt={item.title || ''}
                        width={488}
                        height={488}

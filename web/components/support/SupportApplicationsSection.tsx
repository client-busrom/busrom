...
export const SupportApplicationsSection: React.FC<SupportApplicationsSectionProps> = ({ 
  ...
  const duplicatedItems = useMemo(() => {
    ...
    let set = [...validItems]
    while (set.length < 4 && set.length > 0) {
      set = [...set, ...validItems]
    }
    return [...set, ...set, ...set]
  }, [validItems])
  
  const setWidthVw = useMemo(() => {
    if (validItems.length === 0) return 0
    let total = 0
    const setSize = Math.max(validItems.length, 4)
    for (let i = 0; i < setSize; i++) {
      total += (ITEM_CONFIGS[i % 4].width / DESIGN_WIDTH) * 100
    }
    return total
  }, [validItems.length])
  ...
  const startInfiniteScroll = (currentX: number) => {
    ...
    if (setWidthVw === 0 || isHovering.current) return

    let startVal = currentX % setWidthVw
    if (startVal > 0) startVal -= setWidthVw

    animationRef.current = animate(x, [startVal, startVal - setWidthVw], {
      duration: 40,
      ease: "linear",
      repeat: Infinity,
      repeatType: "loop",
  ...
  useEffect(() => {
    if (validItems.length > 0 && setWidthVw > 0 && !isHovering.current) {
      startInfiniteScroll(x.get())
    }
    return () => animationRef.current?.stop()
  }, [validItems.length, setWidthVw])
  ...
  const handleNext = () => {
    ...
    const jumpDistance = setWidthVw / Math.max(validItems.length, 4)
    animate(x, x.get() - jumpDistance, {
      duration: 0.4,
      ease: [0.22, 1, 0.36, 1],
      onComplete: () => {
  ...
  if (loading) return (
    <div className="py-[120px] flex justify-center items-center bg-[#FBF9F1] min-h-[400px]">
  ...
  return (
    <section id="applications" className="bg-[#FBF9F1] overflow-hidden relative select-none" style={{ paddingTop: vw(20), paddingBottom: vw(100) }}>
      <div className="relative w-full overflow-hidden" style={{ height: vw(922) }}>
        <motion.div 
          className="flex items-start absolute left-0 top-0 h-full"
          style={{ gap: 0, x: translateX }}
        >
          {duplicatedItems.map((item, index) => {
            const styleIdx = index % 4
            const config = ITEM_CONFIGS[styleIdx]
            return (
              <div key={`${item.id || index}-${index}`} className="relative flex-shrink-0" style={{ width: vw(config.width), height: "100%" }}>
      ...
      <button className="absolute left-[2vw] top-3/4 -translate-y-1/2 z-30 group flex items-center justify-center transition-all duration-300" style={{ width: vw(141), height: vw(141) }} onClick={handlePrev}>
      ...
      <button className="absolute right-[2vw] top-3/4 -translate-y-1/2 z-30 group flex items-center justify-center transition-all duration-300" style={{ width: vw(141), height: vw(141) }} onClick={handleNext}>
        <div className="absolute inset-0 rounded-full bg-[#f1ebc8] opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />

...
export async function getMainForm(locale: string = 'en'): Promise<MainFormData> {
  try {
    ...
    return {
      ...
      submittingText: extractLocale(config.submittingText, locale) || 'Submitting...',
      successMessage: extractLocale(config.successMessage, locale) || 'Submitted successfully!',
      errorRequired: extractLocale(config.errorRequired, locale) || 'Please fill in required fields',
      errorNetwork: extractLocale(config.errorNetwork, locale) || 'Network error, please try again',
      errorCaptcha: extractLocale(config.errorCaptcha, locale) || 'Please complete the captcha verification',

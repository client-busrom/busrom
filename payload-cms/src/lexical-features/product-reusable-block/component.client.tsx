// @ts-nocheck
/**
 * ProductReusableBlock Component - Client Side
 *
 * Dedicated for product detail templates.
 */
...
interface ProductReusableBlockComponentProps {
  nodeKey: string
  data: ProductReusableBlockData
}

interface ProductReusableBlock {
  id: string | number
  slug: string
...
export const ProductReusableBlockComponent: React.FC<ProductReusableBlockComponentProps> = ({ nodeKey, data }) => {
  const [editor] = useLexicalComposerContext()
  const { i18n } = useTranslation()
  const lang = i18n?.language === 'zh' ? 'zh' : 'en'

  const [isEditing, setIsEditing] = useState(false)
  const [localData, setLocalData] = useState<ProductReusableBlockData>(data)
  const [reusableBlocks, setReusableBlocks] = useState<ProductReusableBlock[]>([])
  const [selectedBlock, setSelectedBlock] = useState<ProductReusableBlock | null>(null)
  ...
  // Load product reusable blocks
  useEffect(() => {
    const loadReusableBlocks = async () => {
      setIsLoading(true)
      try {
        const response = await fetch('/api/product-reusable-blocks?limit=100&sort=-updatedAt')
        if (response.ok) {
          const data = await response.json()
          setReusableBlocks(data.docs || [])
        }
      } catch (error) {
        console.error('Failed to load product reusable blocks:', error)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  useEffect(() => {
    const loadSelectedBlock = async () => {
      const blockId = typeof localData.productReusableBlock === 'object' ? localData.productReusableBlock?.id : localData.productReusableBlock
      if (!blockId) {
      ...
      try {
        const response = await fetch(`/api/product-reusable-blocks/${blockId}`)
        if (response.ok) {
          const data = await response.json()
          setSelectedBlock(data)
        }
      } catch (error) {
        console.error('Failed to load product reusable block:', error)
      }
    }

    loadSelectedBlock()
  }, [localData.productReusableBlock])
  ...
  const handleSave = () => {
    editor.update(() => {
      const node = $getNodeByKey(nodeKey) as ProductReusableBlockNode
      if (node) {
  ...
  const handleBlockSelect = (blockId: string) => {
    setLocalData({ ...localData, productReusableBlock: { id: blockId } })
  }

  const getBlockEditUrl = (blockId: string | number): string => {
    return `/admin/collections/product-reusable-blocks/${blockId}`
  }
  ...
  const selectedBlockId = typeof localData.productReusableBlock === 'object' ? localData.productReusableBlock?.id : localData.productReusableBlock

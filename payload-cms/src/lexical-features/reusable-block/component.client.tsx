 ...
 * - Filter by block type and status
 * - Link to edit reusable block in admin
...
import { Recycle, ExternalLink, Search, X, Filter } from 'lucide-react'
...
const BLOCK_TYPES = [
  { value: '', label: { en: 'All Types', zh: '全部类型' } },
  { value: 'CTA', label: { en: 'CTA', zh: '行动召唤' } },
...
export const ReusableBlockComponent: React.FC<ReusableBlockComponentProps> = ({ nodeKey, data }) => {
  ...
  useEffect(() => {
    const loadReusableBlocks = async () => {
      setIsLoading(true)
      try {
        const response = await fetch('/api/reusable-blocks?limit=100&sort=-updatedAt')
        if (response.ok) {
  ...
  const filteredBlocks = useMemo(() => {
    return reusableBlocks.filter((block) => {
      ...
      // Type filter
      if (typeFilter && block.blockType !== typeFilter) return false
      // Status filter
      if (statusFilter && block.status !== statusFilter) return false
      return true
    })
  }, [reusableBlocks, searchText, typeFilter, statusFilter, lang])
  ...
  const getBlockEditUrl = (blockId: string | number): string => {
    return `/admin/collections/reusable-blocks/${blockId}`
  }
  ...
  const hasFilters = searchText || typeFilter || statusFilter
  ...
  const selectedBlockId = typeof localData.reusableBlock === 'object' ? localData.reusableBlock?.id : localData.reusableBlock
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            <select
              ...
              style={{
                ...
                backgroundColor: 'white',
                cursor: 'pointer',
                color: typeFilter ? '#1e293b' : '#6b7280',
              }}
            >
              {BLOCK_TYPES.map((bt) => (
                <option key={bt.value} value={bt.value}>{bt.label[lang]}</option>
              ))}
            </select>
          ...
          {isLoading ? (
            ...
            <div style={{ padding: '20px', textAlign: 'center', color: '#6b7280', fontSize: '13px' }}>
              {hasFilters
                ...
                : t('No reusable blocks available', '没有可用的可复用块')
              }
            </div>
          ) : (
        ...
        {selectedBlock && (
          <div style={{
            ...
            {selectedBlock.subtitle && (
              <div style={{ fontSize: '12px', color: '#4b5563', marginBottom: '6px', fontStyle: 'italic' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {selectedBlock ? (
              ...
              <div style={{ display: 'flex', gap: '6px', marginBottom: '6px' }}>
                {selectedBlock.blockType && (
                  <span style={{
                    ...
                    {getBlockTypeLabel(selectedBlock.blockType)}
                  </span>
                )}
            ...
            <div style={{ fontSize: '13px', color: '#ef4444', padding: '8px 0' }}>
              {blockId
                ? `⚠️ ${t('Reusable block not found', '可复用块未找到')}`
                : `⚠️ ${t('No reusable block selected', '未选择可复用块')}`
              }
            </div>
          )}
        </div>

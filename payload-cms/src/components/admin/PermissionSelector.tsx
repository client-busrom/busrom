...
const CATEGORY_LABELS: Record<string, { en: string; zh: string }> = {
  USER: { en: "Users & Access", zh: "用户与权限" },
  NAVIGATION: { en: "Navigation", zh: "导航管理" },
  WEBSITE_PAGES: { en: "Website Pages", zh: "网站页面管理" },
  PRODUCTS: { en: "Products", zh: "产品管理" },
  CONTENT: { en: "Content Management", zh: "内容管理" },
  MEDIA: { en: "Media Library", zh: "媒体库" },
  FORMS: { en: "Forms", zh: "表单管理" },
...
const RESOURCE_LABELS: Record<string, { en: string; zh: string }> = {
  ...
  PRODUCT: { en: "Product Links", zh: "产品链接整合页" },
  PRODUCT_SERIES: { en: "Product Details", zh: "产品详解整合页" },
  PRODUCT_ATTRIBUTE: { en: "Product Attributes", zh: "产品链接页属性" },
  PRODUCT_TEMPLATE: { en: "Product Templates", zh: "产品链接模版页" },
  PRODUCT_REUSABLE_BLOCK: {
    en: "Product Reusable Blocks",
    zh: "产品链接复用块",
  },
  SERIES_TEMPLATE: { en: "Series Templates", zh: "产品详解模版页" },
  SERIES_REUSABLE_BLOCK: { en: "Series Reusable Blocks", zh: "产品详解复用块" },
  SERIES_INTRO_ITEM: { en: "Series Intro Items", zh: "系列介绍" },
  HERO_BANNER_ITEM: { en: "Hero Items", zh: "首页首屏轮播图" },
  PAGE: { en: "Pages", zh: "页面" },
  ...
  REUSABLE_BLOCK: { en: "Reusable Blocks", zh: "可复用块" },
  DOCUMENT_TEMPLATE: { en: "Section Templates", zh: "页面板块模板库" },
  NAVIGATION_MENU: { en: "Nav Menus", zh: "导航菜单" },
  ...
  FORM_SUBMISSION: { en: "Form Submissions", zh: "表单提交记录" },
  HOME_CONTENT: { en: "Homepage Sections", zh: "首页组件管理" },
  FOOTER: { en: "Global Footer", zh: "全站页脚配置" },
  HOMEPAGE_GLOBAL: { en: "Homepage Global", zh: "首页全局设置" },
  SITE_CONFIG: { en: "Site Config", zh: "站点配置" },
  SEO_SETTING: { en: "SEO Settings", zh: "SEO设置" },
  CUSTOM_SCRIPT: { en: "Custom Scripts", zh: "自定义脚本" },
  EMAIL_CONFIG: { en: "Email Config", zh: "邮件配置" },
  CONTACT_CONFIG: { en: "Contact Settings", zh: "联系配置" },
  SOCIAL_CONFIG: { en: "Social Links", zh: "社交配置" },
  TRANSLATION_CONFIG: { en: "Translation Settings", zh: "翻译配置（全局）" },
  SHOP_PAGE_CONFIG: { en: "Shop Preview", zh: "Shop 列表页管理" },
  PRELOADER_CONFIG: { en: "Preloader Config", zh: "加载动画配置" },
  TEMPLATE_CATEGORY: { en: "Template Categories", zh: "组件模版库分类集合" },
};
...
export const PermissionSelector: React.FC<PermissionSelectorProps> = ({
  ...
  const rolesValue = useMemo(() => {
    ...
    if (
      typeof rolesFieldValue === "object" &&
      rolesFieldValue !== null &&
      "id" in rolesFieldValue
    ) {
  ...
  const isEffectivelySelected = useCallback(
    (permId: string) => {
      return (
        selectedIds.has(String(permId)) || rolePermissionIds.has(String(permId))
      );
    },
  ...
  const toggleCategory = useCallback(
    (category: string) => {
      ...
      const allToggleableSelected = toggleablePerms.every((p) =>
      ...
      if (allToggleableSelected) {
        ...
        newValue = (value || []).filter(
          (id) => !toggleableIds.map(String).includes(String(id)),
        );
      } else {
  ...
  const selectAll = useCallback(() => {
    if (isUserCollection) {
      const nonRolePerms = permissions.filter(
        (p) => !rolePermissionIds.has(String(p.id)),
      );
  ...
  const messages = {
    ...
    cannotCancelTitle: {
      en: "This permission is from role, cannot cancel",
      zh: "此权限来自角色，不可取消",
    },
  };
  ...
  return (
    <div className="permission-selector" style={{ marginBottom: "20px" }}>
      ...
      {isUserCollection && rolePermissionIds.size > 0 && (
        ...
        >
          <strong>
            {t(messages.roleGranted)} {rolePermissionIds.size}{" "}
            {t(messages.permissions)}
          </strong>
          {t(messages.cannotCancel)}
          {additionalAvailable > 0 ? (
            <span>
              。{t(messages.canAdd)} {additionalAvailable}{" "}
              {t(messages.additionalPerms)}。
            </span>
          ) : (
      ...
      >
        <span style={{ fontWeight: 500 }}>
          {isUserCollection ? (
            <>
              {t(messages.totalPerms)}: {totalEffectivePermissions} /{" "}
              {permissions.length}
              {rolePermissionIds.size > 0 && (
                ...
                >
                  ({t(messages.fromRole)}: {rolePermissionIds.size},{" "}
                  {t(messages.additional)}: {selectedIds.size})
                </span>
              )}
            </>
          ) : (
            <>
              {t(messages.selected)} {selectedIds.size} / {permissions.length}{" "}
              {t(messages.permissions)}
            </>
          )}
        </span>
      ...
      {sortedCategories.map((category) => {
        ...
        const roleInCategory = isUserCollection
          ? categoryPerms.filter((p) => rolePermissionIds.has(String(p.id)))
              .length
          : 0;
        const directInCategory = categoryPerms.filter(
          (p) =>
            selectedIds.has(String(p.id)) &&
            !rolePermissionIds.has(String(p.id)),
        ).length;

        // Check if all permissions in category are from role (no toggleable ones)
        const allFromRole =
          isUserCollection && roleInCategory === categoryPerms.length;
          ...
          >
            ...
            >
              ...
              <input
                ...
                ref={(el) => {
                  if (el)
                    el.indeterminate = isPartiallySelected && !allFromRole;
                }}
                onChange={(e) => {
                  e.stopPropagation();
                  if (!allFromRole) {
              ...
              <span style={{ fontWeight: 600, flex: 1 }}>
                ...
                {isUserCollection && roleInCategory > 0 && (
                  ...
                  >
                    {allFromRole
                      ? `✓ ${t(messages.allFromRole)}`
                      : `${t(messages.fromRoleLabel)}: ${roleInCategory}`}
                  </span>
                )}
              </span>
            </div>
                    ...
                    >
                      {perms.map((perm) => {
                        const isRolePermission =
                          isUserCollection &&
                          rolePermissionIds.has(String(perm.id));
                        const isDirectPermission = selectedIds.has(
                          String(perm.id),
                        );
                        const isChecked =
                          isRolePermission || isDirectPermission;
                        ...
                        return (
                          ...
                          >
                            ...
                            <span
                              style={{
                                color: isRolePermission
                                  ? "var(--theme-info-700)"
                                  : undefined,
                              }}
                            >
                              {ACTION_LABELS[perm.action]
                                ? t(ACTION_LABELS[perm.action])
                                : perm.action}
                            </span>
                            {isRolePermission && (
                              <span

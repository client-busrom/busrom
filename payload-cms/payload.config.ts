...
export default buildConfig({
  ...
  db: postgresAdapter({
    pool: {
      ...
      // For AWS RDS with self-signed certificates (Skip SSL for local Postgres even in production mode)
      ...((process.env.NODE_ENV === 'production' && process.env.DATABASE_URI && !process.env.DATABASE_URI.includes('localhost') && !process.env.DATABASE_URI.includes('127.0.0.1')) && {
        ssl: {
    ...
    push: process.env.NODE_ENV !== 'production', // Disabled in production - causes CPU exhaustion on 1 vCPU Fargate
  }),
  ...
  editor: lexicalEditor({
    features: ({ defaultFeatures }) => {
      ...
      return [
        ...
        FaqCarouselFeature(), // Custom Feature FAQ 轮播推荐 - WYSIWYG

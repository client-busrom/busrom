/**
 * 批量创建产品脚本 - 通过 GraphQL API
 * 需要 CMS 在本地运行 (localhost:3000)
 *
 * Usage:
 * node scripts/seed-products-via-graphql.js
 */

const CMS_URL = 'http://localhost:3000/api/graphql'
...
async function main() {
  ...
  if (mediaResult.errors) {
    console.error('获取 Media 失败:', mediaResult.errors)
    return
  }

  const allMedia = mediaResult.data.mediaFiles
  ...
  for (const series of seriesConfig) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      try {
        ...
        if (existingResult.data?.products?.length > 0) {
        ...
        // 如果有图片，连接图片
        if (mainImage) {
          productData.mainImage = { connect: { id: mainImage.id } }
        }
        if (showImage) {
          productData.showImage = { connect: { id: showImage.id } }
        }

        const result = await graphqlRequest(mutation, { data: productData })

/**
 * Seed Script: HeroBannerItem (using GraphQL)
 *
...
async function main() {
  ...
  // Get first available media as placeholder
  const mediaQuery = `
    query {
      mediaFiles(take: 1) {
        id
  ...
  `
  const mediaData = await graphqlRequest(mediaQuery)
  const placeholderMedia = mediaData.mediaFiles[0]
  ...
  // Clear existing items (we'll do this manually or skip for now)
  console.log('ℹ️  Skipping deletion of existing items (do manually if needed)')
  ...
  for (const item of heroBannerData) {
    ...
    try {
      const result = await graphqlRequest(createMutation, { data })
      created++

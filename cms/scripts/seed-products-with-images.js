...
const glassThicknessList = ['6mm', '8mm', '10mm', '12mm', '15mm', '19mm']
const loadCapacityList = ['50kg', '80kg', '100kg', '150kg', '200kg']
...
async function main() {
  ...
  for (const series of seriesConfig) {
    ...
    const whiteImages = seriesMedia.filter(m => m.filename && m.filename.includes('_white_'))
    const sceneImages = seriesMedia.filter(m => m.filename && m.filename.includes('_scene_'))

    const names = productNames[series.skuPrefix] || []

    for (let i = 0; i < 10; i++) {
      const name = names[i] || { en: `${series.seriesNameEn} Model ${i + 1}`, zh: `${series.seriesNameZh}型号${i + 1}` }
      const material = randomPick(materials)
      const finish = randomPick(finishes)
      const glass = randomPick(glassThicknessList)
      const load = randomPick(loadCapacityList)
      ...
      try {
        const product = await prisma.product.create({
          data: {
            ...
            mainImage: mainImage ? { id: mainImage.id } : null,
            showImage: showImage ? { id: showImage.id } : null,

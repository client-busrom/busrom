/**
 * Update SeriesIntro data directly via Prisma
 *
 * Usage:
 *   npx tsx scripts/update-series-intro-prisma.ts
 */
...
async function main() {
  console.log('Updating SeriesIntro data via Prisma...\n')

  // First, fetch all ProductSeries to get their IDs
  const productSeries = await prisma.productSeries.findMany({
  ...
  const existingItems = await prisma.seriesIntro.findMany({
  ...
  const existingMap: Record<string, typeof existingItems[0]> = {}
  ...
  for (let i = 0; i < seriesIntroData.length; i++) {
    ...
    if (existing) {
      // Update existing
      try {
        await prisma.seriesIntro.update({
      ...
      // Create new
      try {
        await prisma.seriesIntro.create({
          data: {
